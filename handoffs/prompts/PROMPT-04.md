...
## 2. Mid-session correction — the Vercel build command (23 Aug 2026, arrived while the branch was building, before the PR was opened)

Before you open the PR: the preview build on this branch is failing on Vercel with "Module not found: Can't resolve @zcashreveal/types". apps/web/vercel.json pins buildCommand to "pnpm --filter @zcashreveal/content build && next build", which does not build the types package that fold 1 just added as a dependency. It passes locally only because pnpm build goes through turbo, whose build task has dependsOn ['^build']; Vercel runs the literal string and bypasses turbo. Change buildCommand to "pnpm turbo run build --filter=@zcashreveal/web" so the dependency graph resolves itself and cannot drift again, extend scripts/check-vercel-config.mjs to assert that form, and add a §5 assertion that the deployed preview reaches READY. Say in §7 which deployment id proved it.

## 3. Mid-session correction — the deleted Vercel projects, and the viewing key (23 Aug 2026, arrived during the gate round, before the PR was opened)

**THE TWO OLD VERCEL PROJECTS NO LONGER EXIST.** The operator deleted z-cash-reveal-dashboard and z-cash-reveal-dashboard2 on 23 Aug 2026; zecreveal (Root Directory apps/web) is now the only project on the account, and L2 confirmed that from the Vercel API. Add a deliverable: correct docs/2.0/DEPLOY-2.0.md (nine references) and the root DEPLOY.md (five) so they describe the one project that exists, and drop the two now-moot operator clicks from the handoffs/README.md table — "delete the orphaned project" and "move the root vercel.json settings into z-cash-reveal-dashboard2". **Do NOT rewrite handoffs 00 to 02, docs/2.0/BRANCH-CLEANUP.md, HANDOFF-2026-08-22-v2.md or the v0.2 runbook**: those are historical records of what was true when they were written, and LEDGER-03 Q3's sweep rule is about claims of fact the site states, not about a log of past state. Note in section 7 which files you corrected and which you deliberately left. HANDOFF-11's cutover scope now shrinks to pointing a domain at zecreveal, which is a note for that handoff, not work for this one.

**THE VIEWING KEY MUST NOT BE ABLE TO LEAVE THE TAB, AND THE UNCONTROLLED-INPUT FIX IS ONLY HALF OF IT.** Your finding that React serialises a controlled value into the DOM attribute is exactly right and the fix is correct. Please close the rest of the surface with a §5 assertion, both polarities: with the reveal page loaded, typing a well-formed viewing key produces ZERO network requests whose URL, headers or body contain any substring of it (Playwright request interception over the whole session, including the CSP report-uri if one is configured); the field carries autocomplete='off', spellcheck='false', no name attribute, and is not inside a form element that can submit; and the key appears in no attribute of any element in the serialised DOM after typing. **Fail side: bind the value back to React state in a scratch build and watch the DOM assertion fail.** This is the one promise on the whole site that cannot be walked back after it is broken once, so it should be measured rather than reasoned about.
