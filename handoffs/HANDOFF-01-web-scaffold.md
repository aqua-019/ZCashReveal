...
- Env names are reserved now, wired later: public `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_WS_URL`, `NEXT_PUBLIC_SNAPSHOT_URL`, `NEXT_PUBLIC_DATA_MODE`; server-only (never `NEXT_PUBLIC_`) `SNAPSHOT_REDIS_REST_URL`, `SNAPSHOT_REDIS_REST_TOKEN`, `SNAPSHOT_REDIS_URL` for the Vercel-managed Redis snapshot store (HANDOFF-09 writes it, HANDOFF-11 reads it). *(Corrected by HANDOFF-05: those three names are injected by nothing. The store is connected under the variable prefix `SNAPSHOT_REDIS` and the real names are `SNAPSHOT_REDIS_KV_REST_API_URL`, `SNAPSHOT_REDIS_KV_REST_API_READ_ONLY_TOKEN`, `SNAPSHOT_REDIS_KV_REST_API_TOKEN`, `SNAPSHOT_REDIS_KV_URL` and `SNAPSHOT_REDIS_REDIS_URL` — see `docs/2.0/SNAPSHOT.md` §3. The sentence is left standing because this handoff is closed and its text is the record of what was specified at the time; the correction is appended rather than substituted.)* `apps/web/.env.example` lists all of them with one-line comments; nothing reads the `SNAPSHOT_*` names yet.
