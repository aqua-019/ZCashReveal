...
track: Integration
depends_on: 04, 05, 09, 09a, 09b, 10
...
- **THE CUTOVER MAY NOT SHIP A NULL ANALYSIS PANEL** (LEDGER-09 Q4). `SnapshotV1` permits `residual`, `drain`, `migrationHist` and `neffSeries` to be `null`, and `docs/2.0/SNAPSHOT.md` §8.1 defines that null as "not measured" rather than as a zero. **That is the honest TYPE and it does not make a null PANEL honest on a production page.** The precedent is this project's own, from LEDGER-05 Q2: `/api/pools` answers 503 naming the four blocks it cannot serve rather than serving four empty ones, because a page that serves four empty blocks is claiming to have looked and found nothing. Four null panels on a live cutover is that same claim in a different shape. HANDOFF-09a is the prerequisite that removes the condition - it moves the three estimators into a package the publisher may depend on and composes them at its composition root - which is why `depends_on` above names it. **The third option is rejected explicitly so nobody re-derives it:** shipping the cutover with null panels and a "not measured" surface is tempting because the type already models it honestly, and it is still no, because the cutover is the production promotion and it is the one gate where "the next handoff fixes it" becomes "the public site says nothing about four of the things it exists to measure".
