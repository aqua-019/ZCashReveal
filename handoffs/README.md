...
| 05 | [Gateway REST read API v2 + hardening (Zebra address-index RPCs with a cache)](HANDOFF-05-gateway-api.md) | session-designated | Data | 00 (uses the DTOs from 04 if merged; otherwise defines them) | `in-progress` |
...
Web (01→04), Data (05→09) and Infra (10) are independent once 00 is closed. 00, 01, 02, 03 and 04 are `closed` (PRs #31 `0031d7c`, #32 `0ed75ad`, #33 `8badaa3`, #34 `47ff9d5`, #35 `8d97e25`), which closes the Web track's first pass — 04 was the last Web handoff in the sequence, so the Web track has nothing `open`. 05 is `in-progress` — the Data track's handoff, owned by the session that reconciled it. 06–09 stay `queued` behind 05's track position: a track holds one active handoff, and `in-progress` occupies that position exactly as `open` does, which is why 06 does not open now even though its only dependency (00) is closed. 10 is `open` and unclaimed on purpose — the Infra track, which needs its own session told it owns HANDOFF-10 (LEDGER-02 Q1). 11 and 12 integrate and stay `queued` until 05, 09 and 10 close. 13 is plan-only.
