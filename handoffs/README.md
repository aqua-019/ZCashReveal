...
| 04 | [ZEC Tracking UI in fixture mode — search, mempool, address, tx, pools, reveal](HANDOFF-04-tracking-ui.md) | session-designated | Web | 01, 02 (03 optional) | `shipped` |
...
Web (01→04), Data (05→09) and Infra (10) are independent once 00 is closed. 00, 01, 02 and 03 are `closed` (PRs #31 `0031d7c`, #32 `0ed75ad`, #33 `8badaa3`, #34 `47ff9d5`). 04 is `in-progress` — the Web track's handoff, owned by the session that reconciled it, and the last Web handoff in the sequence. The Data and Infra tracks stay at their lowest unblocked handoff: 05 and 10 are `open` and unclaimed on purpose (LEDGER-02 Q1). 06–09 stay `queued` behind 05's track position. 11 and 12 integrate and stay `queued` until 04, 05, 09 and 10 close. 13 is plan-only.
...
| 03 — **now blocking** | **turn on Protection Bypass for Automation on the `zecreveal` project**, or drop Deployment Protection for preview deployments. Three revolutions have now ended with the route checklist UNVERIFIED over the wire, and LEDGER-03 Q1 moves the authoritative Lighthouse measurement to the deployed page, which nobody - L2, CI or L3 - can reach while every preview returns 302 to the SSO endpoint. Until this click, HANDOFF-04's A10 records a container number instead (LEDGER-03). **HANDOFF-04 found a second wall behind this one**: the session container's egress proxy refuses the preview host outright (`curl` returns `CONNECT tunnel failed, response 403`, not a 302 to SSO), so lifting Deployment Protection alone will not let a session measure the preview - it also needs the host allowed, or the measurement taken by the operator, or a custom domain, which `all_except_custom_domains` already exempts (LEDGER-04 Q3) |
