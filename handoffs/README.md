...
| 09 | [Turnstile accounting, migration lens, Ironwood birth, snapshot publisher](HANDOFF-09-instruments-snapshot.md) | session-designated | Data | 06, 08 (both closed) | `in-progress` |
| 10 | [Infra: Zebra 6.2.x compose, VPS runbook, tunnel, DEPLOY-2.0](HANDOFF-10-infra.md) | `claude/handoff-08-completion-wngbjj` | Infra | 00 | `closed` (PR #43 `8679e03`) |
...
Web (01→04), Data (05→09) and Infra (10) are independent once 00 is closed. 00 through 08 and 10 are `closed` (PRs #31 `0031d7c`, #32 `0ed75ad`, #33 `8badaa3`, #34 `47ff9d5`, #35 `8d97e25`, #36 `31d5000`, #37 `eba5b03`, #38 `861c460`, 08's four at `4386e98` / `09b9e9c` / `9393100` / `4ae0796`, and 10 at `8679e03`). The Web track's first pass is finished - 04 was the last Web handoff in the sequence, so the Web track has nothing `open` and nothing `queued`. **The HANDOFF-09 session's RECONCILE closed 08 and 10 and moved three rows**, and the reasoning is recorded here because the reconcile rule is mechanical and its output is not obvious. 08 and 10 close because PRs #42 and #43 are merged into `main` at `4ae0796` and `8679e03`. 09 takes the Data track's active position and is `in-progress`. **12 opens, and 11 does not, which is the part worth reading twice.** The rule opens the lowest-numbered `queued` handoff on a track *whose `depends_on` are all closed*, not the lowest-numbered `queued` handoff outright: 11 depends on 04, 05, 09 and 10, and 09 is not closed, so 11 stays `queued` and is stepped over; 12 depends on 06, 07 and 08, all three of which are now closed, so 12 is the Integration track's one open handoff. That is not 12 jumping the queue - it is the queue being ordered by dependency rather than by number, and it means the Integration track can start on the runtime wiring while the Data track finishes the instruments. The Infra track has nothing else queued - 10 was its only handoff - so it goes quiet until 11 integrates. 13 is plan-only and waits on 11.
