...
| 03 | [The Record — Splash, Beware, Contradictions, Timeline, Network, Method, Flows, Sources](HANDOFF-03-record-pages.md) | session-designated | Web | 01, 02 | `in-progress` |
...
Web (01→04), Data (05→09) and Infra (10) are independent once 00 is closed. 00, 01 and 02 are `closed` (PRs #31 `0031d7c`, #32 `0ed75ad`, #33 `8badaa3`), so the Web track has advanced to 03, which the HANDOFF-03 session owns and marks `in-progress`. 04's `depends_on` (01, 02) are both closed and it does not depend on 03, so it releases as the Web track's `open` handoff; a session that takes it works alongside 03's unmerged PR in `apps/web` and should expect to merge `main` before it finishes. The Data and Infra tracks stay at their lowest unblocked handoff: 05 and 10 are `open` and unclaimed on purpose (LEDGER-02 Q1). 06–09 stay `queued` behind 05's track position. 11 and 12 integrate. 13 is plan-only.
