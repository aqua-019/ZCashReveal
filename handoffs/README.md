...
| 06 | [Indexer: four pools + migration 003 + post-NU6.3 invariants](HANDOFF-06-four-pools.md) | `claude/new-session-s4er6f` | Data | 00 | `shipped` |
...
Web (01→04), Data (05→09) and Infra (10) are independent once 00 is closed. 00, 01, 02, 03, 04 and 05 are `closed` (PRs #31 `0031d7c`, #32 `0ed75ad`, #33 `8badaa3`, #34 `47ff9d5`, #35 `8d97e25`, #36 `31d5000`). The Web track's first pass is finished — 04 was the last Web handoff in the sequence, so the Web track has nothing `open` and nothing `queued`. 05 merged, which freed the Data track's active position, and 06 took it and is now `shipped`: its PR is opened and stopped there, and it holds that position until the operator merges it and the next session's RECONCILE sets it `closed`. 07, 08 and 09 stay `queued` behind it — a track holds one active handoff, and `shipped` occupies it exactly as `open` and `in-progress` do. 10 is `open` and unclaimed on purpose — the Infra track, which needs its own session told it owns HANDOFF-10 (LEDGER-02 Q1). 11 and 12 integrate and stay `queued` until 06, 09 and 10 close. 13 is plan-only.
