...
3.5 **Fee → logical actions (hard).** ZIP 317: `fee = 5,000 zat × max(2, L)`, `L = max(t_in, t_out) + 2·nJoinSplit + max(nSpendsSapling, nOutputsSapling) + nActionsOrchard (+ nActionsIronwood)`. With the transparent counts public, `L` bounds the shielded arity exactly; a non-conventional fee is itself a wallet fingerprint (v0.2 `isZip317ConventionalFee`).
