...
where `inSize` and `outSize` are the summed serialised sizes of the transparent inputs and of the transparent outputs, and 150 and 34 are the sizes ZIP 317 fixes for a standard P2PKH input and output. Taken from the canonical implementation rather than from a summary of it: Zebra `zebra-chain/src/transaction/unmined/zip317.rs:160-173`. With the transparent side public, `L` bounds the shielded arity exactly; a non-conventional fee is itself a wallet fingerprint (v0.2 `isZip317ConventionalFee`).

 - *The count form, kept beside it as the approximation it is.* Through HANDOFF-05 this section gave `L_p2pkh = max(t_in, t_out) + 2·nJoinSplit + max(nSpendsSapling, nOutputsSapling) + nActionsOrchard (+ nActionsIronwood)` — the same rule with its transparent term replaced by **counts**. It is exact if and only if every transparent input and output is a standard P2PKH (148 and 34 bytes), which is nearly every transaction on the chain and is why the divergence stayed invisible for so long. It survives in the code as `zip317LogicalActionsP2pkhApproximation` (`packages/zec-types/src/zip317.ts`), so `/method` can show a reader both forms, and it is never used to decide whether a fee was conventional.
...
3.6 **Dummy-padding policy (soft, wallet table).** Orchard/Ironwood action counts are padded (≥ 2; some wallets pad to even counts or fixed sizes). The observed `nActions`, `nSpends`/`nOutputs` for Sapling, `expiryDelta = nExpiryHeight − height` (zcashd 20, Zashi/Zodl 40, others vary), version group and anchor age are matched against a **fingerprint table** (Zodl/Zashi, Ywallet ≤ 1.15.3, Zingo, Zkool, Vizor, Cake, NozyWallet, zcashd/Zallet, Ledger/Keystone). Output: `likelyWallet` with a likelihood ratio; never an identity.
...
3.9 **Migration lens (ZIP 318, hard amounts / soft sessions).** Each Orchard→Ironwood migration spends one Orchard note into one Ironwood output with the net amount public and quantised to `n × 10^k, n ∈ {1,2,5}`, cap 10,000 ZEC, dust < 0.01 ZEC stranded. A wallet's balance `B` decomposes canonically, so a migration **session** (a burst of canonical denominations within a scheduling window) bounds the *number of notes* (`≥ ⌈B/10,000⌉`) and the *set of wallets* (`≤ number of denomination runs`). Reported as distributions and counts per window — **never** as "wallet W migrated B".
