...
| Ledger | **none** | named only in the v2.0 **mockups** (`docs/2.0/mockups/zecreveal-2.0-mockups-v2.html`, the §3.6 estimator row), and absent from the corpus and from the plan. The plan's only "Ledger" is the Turnstile Ledger, an unrelated surface | no |
