...
| Orphan project `z-cash-reveal-dashboard` | "delete it" | **Deleted** by the operator on 23 Aug 2026, along with `z-cash-reveal-dashboard2`. `zecreveal` (Root `apps/web`) is now the only project on the account, so the red Vercel check that appeared on every PR from HANDOFF-00 to HANDOFF-03 is gone and a red check now means the branch (corrected by HANDOFF-04). |
...
**3.4 Migration lens (ZIP 318).** Each migration tx spends exactly one Orchard note → one Ironwood output, with the net amount public and quantised to `n × 10^k, n ∈ {1,2,5}`, the largest crossing being **10,000 ZEC** (ZIP 318's `DENOM_CAP` is 10,000 ZEC *plus the canonical fee* and bounds the funding note, not the crossing — see TRACKING-MATH §3.9; ZIP 318 is status Draft). Per block: count, sum, denomination histogram `M_h[d]`. Derived, with stated assumptions: the *minimum* number of notes `≥ ⌈amount / 10,000 ZEC⌉`; an *upper bound* on distinct migrating wallets per window `≤ Σ counts` (no lower bound is claimable). Bucketing + scheduling are heuristic privacy defences, so the lens reports **distributions only**, never wallet attributions.
...
- `apps/gateway` gains a **REST read API** (`/v2/address/:a`, `/v2/tx/:id`, `/v2/block/:h`, `/v2/pools`, `/v2/flows`) over Postgres; WS stays for the mempool.
...
**Delete now**: 20 stale `claude/*` branches (19 merged, 1 not) + the 2 merged `feat/*` branches. ~~the orphan Vercel project~~ - both v0.2 Vercel projects were deleted on 23 Aug 2026. Corrected from 22 by HANDOFF-01: `docs/2.0/BRANCH-CLEANUP.md` is generated from live git and is authoritative (LEDGER-00 Q3).
...
**New Vercel project**: yes — `zecreveal` with Root Directory `apps/web`, Framework Next.js, env `NEXT_PUBLIC_API_URL` / `NEXT_PUBLIC_WS_URL`. Created 23 Aug 2026 (`prj_rNTLvGWnz92w5qcvROBchPUfdhIR`). The plan said to keep `z-cash-reveal-dashboard2` in mock mode until the cutover and then delete it; the operator deleted it early, on the same day, so `zecreveal` is the only project and the cutover has no project to retire.
