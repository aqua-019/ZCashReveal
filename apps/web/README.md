...
Repo-wide gates (all four must pass before a PR opens):

```bash
pnpm -r test && pnpm typecheck && pnpm lint && pnpm --filter @zcashreveal/content validate
...
which is `NODE_ENV === "development"` **or** `NEXT_PUBLIC_DATA_MODE === "fixture"`
(`src/lib/env.ts`). On a deployed environment with `NEXT_PUBLIC_DATA_MODE=snapshot` it returns
404. The same flag gates the `window.__zr` instrumentation described below.
...
`next dev`, or in a production build running with `NEXT_PUBLIC_DATA_MODE=fixture`. If
`window.__zr` is `undefined`, you are on a snapshot build and this procedure cannot run; restart
with `NEXT_PUBLIC_DATA_MODE=fixture`.
