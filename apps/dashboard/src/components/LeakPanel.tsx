...
function Intro() {
  return (
    <div className="px-8 py-12 overflow-y-auto h-full">
      ...
      <h1
        className="display mt-4 text-[72px] leading-[0.9] text-ink"
        style={{ letterSpacing: "-0.03em" }}
      >
        Shielded{" "}
        <span className="display-italic text-[var(--color-zec-gold)]">≠</span>{" "}
        Silent.
      </h1>
      <p className="mt-6 max-w-[52ch] text-[15px] leading-[1.55] text-[var(--color-ink-muted)]">
        ...
        <em className="display-italic text-ink">metadata</em>. Every shielded
        transaction publishes four classes of public data on-chain — and every
        one of them leaks.
      </p>

      <div className="mt-10 grid grid-cols-1 gap-4 max-w-[640px]">
        <LeakRow
          icon={<IconNullifier size={16} />}
          field="nullifiers"
          desc="Per-note spend markers. Globally unique. Reveal spent-set growth and timing clusters."
        />
        <LeakRow
          icon={<IconValueBalance size={16} />}
          field="valueBalance"
          desc="Net value flowing between transparent and shielded pools. Nonzero ⇒ boundary crossing fully public."
        />
        <LeakRow
          icon={<IconAnchor size={16} />}
          field="anchors"
          desc="Merkle root of the note commitment tree at spend time. Narrows the window during which the spent note could have entered."
        />
        <LeakRow
          icon={<IconCommitment size={16} />}
          field="commitments"
          desc="New note outputs added to the tree. Counts and timing leak per transaction."
        />
      </div>
    ...
    <header>
      <div className="flex items-start justify-between gap-6">
        <div className="min-w-0 flex-1">
          ...
          <h2
            className="display mt-4 text-[44px] leading-[0.95]"
            style={{ color: LEAK_CLASS_COLOR[leakClass], letterSpacing: "-0.025em" }}
          >
      ...
      <div className="mt-5 grid grid-cols-4 gap-4">
        ...
        <Stat label="Wallet">
          <span className="text-ink">
            {report.fingerprint.likelyWallet.toLowerCase().replace(/_/g, " ")}
          </span>
        </Stat>
      </div>
    </header>
...
function Stat({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1">
      <span className="mono text-[9px] uppercase tracking-[0.18em] text-[var(--color-ink-dim)]">
        {label}
      </span>
    ...
    return (
      <section>
        ...
        <p className="mono text-[11px] text-[var(--color-ink-muted)]">
          No leak findings recorded for this transaction.
        </p>
      </section>
    );
  }
    ...
    <section>
      ...
      <ul className="space-y-2">
        {findings.map((f, i) => (
          <li
            key={i}
            className="glass flex items-start gap-3 p-3 rounded-sm"
            style={{
              borderLeftColor: SEVERITY_COLOR[f.severity as Severity],
              borderLeftWidth: 2,
            }}
          >
            <span
              className="pill shrink-0 mt-0.5"
              style={{ color: SEVERITY_COLOR[f.severity as Severity] }}
            >
        ...
        <div className="flex items-center gap-2 mono text-[11px] text-ink">
          {v.direction === "DEPOSIT" && (
            <>
              ...
              <span className="text-[var(--color-ink-muted)]">
                — value flowing into the shielded pool from transparent inputs
              </span>
            </>
          )}
          {v.direction === "WITHDRAWAL" && (
            <>
              ...
              <span className="text-[var(--color-ink-muted)]">
                — value flowing out of the shielded pool to transparent outputs
              </span>
            </>
          )}
...
function ValueCell({ pool, value }: { pool: string; value: bigint }) {
  ...
  return (
    <div className="glass p-4 rounded-sm">
      <div className="mono text-[9px] uppercase tracking-[0.18em] text-[var(--color-ink-dim)]">
        {pool} pool
      </div>
      ...
      <div className="mt-1 mono text-[10px] text-[var(--color-ink-dim)]">
        {isZero ? "z → z (no boundary)" : value > 0n ? "leaving pool" : "entering pool"}
      </div>
    </div>
  );
}
                ...
                <td className="px-3 py-2 text-right">
                  {s.anchorDepthBlocks !== null ? (
                    <span className={clsx(s.isRecentAnchor ? "text-[var(--color-medium)]" : "text-[var(--color-ink)]")}>
                      {s.anchorDepthBlocks}
                      <span className="text-[var(--color-ink-dim)] ml-1">blk</span>
                    </span>
                  ) : (
          ...
          {outs.slice(0, 16).map((o, i) => (
            <li key={i} className="flex items-center justify-between text-[var(--color-ink-muted)]">
              <span className="flex items-center gap-2">
                <IconCommitment size={10} className={
                  o.pool === "orchard" ? "text-[var(--color-zec-gold)]" : "text-[var(--color-ink-dim)]"
                } />
    ...
    <section>
      ...
      <div className="glass rounded-sm p-4">
        ...
        <dl className="grid grid-cols-2 gap-y-2 mono text-[11px]">
          ...
          <Datum k="Expiry Δ" v={f.expiryDelta !== null ? `${f.expiryDelta} blocks` : "—"} />
  ...
  return (
    <section>
      ...
      <p className="text-[12px] leading-[1.5] text-[var(--color-ink-muted)] mb-5 max-w-[68ch]">
        ...
        <em className="display-italic text-ink">not</em> hide the per-note
        cryptographic identifiers — every spend publishes a unique{" "}
  ...
  return (
    <div className="glass rounded-sm p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="display-italic text-[14px] text-ink">
          {isSender ? "inputs →" : "← outputs"}
        </span>
      </div>

      <IdentityBlock
        label="Transparent address"
        empty="no transparent side"
        emptySubtle
        items={transparentAddresses.map((a) => ({
          primary: a,
          tag: "t-addr",
      ...
      {isSender && (
        <IdentityBlock
          label="Nullifier (sender pseudonym)"
          empty="no shielded spend"
          emptySubtle
          items={nullifiers.map((n) => ({
            primary: shortHex(n.value, 12, 8),
            tag: n.pool,
            color: "var(--color-zec-gold)",
            public: true,
          }))}
        />
      )}
      ...
      {!isSender && (
        <IdentityBlock
          label="Note commitment (recipient pseudonym)"
          empty="no shielded output"
          emptySubtle
          items={commitments.map((c) => ({
            primary: shortHex(c.value, 12, 8),
            tag: c.pool,
            color: "var(--color-zec-gold)",
            public: true,
          }))}
      ...
      {((isSender && nullifiers.length > 0) ||
        (!isSender && commitments.length > 0)) && (
        <div className="border-t border-[var(--color-edge)] pt-3">
          <div className="mono text-[10px] uppercase tracking-[0.15em] text-[var(--color-ink-dim)] mb-1">
            Wallet address
          </div>
...
function IdentityBlock({
  ...
  empty: string;
  emptySubtle: boolean;
  items: Array<{ primary: string; tag: string; color: string; public: boolean }>;
}) {
  return (
    <div>
      <div className="mono text-[10px] uppercase tracking-[0.15em] text-[var(--color-ink-dim)] mb-1.5">
        {label}
      </div>
      {items.length === 0 ? (
        <div
          className={clsx(
            "mono text-[11px]",
            emptySubtle ? "text-[var(--color-ink-dim)]" : "text-[var(--color-ink-muted)]",
          )}
        >
        ...
        <ul className="space-y-1">
          {items.map((it, i) => (
            <li key={i} className="flex items-center justify-between gap-2 mono text-[11px]">
              ...
              <span className="text-[var(--color-ink-dim)] shrink-0 text-[10px] uppercase tracking-[0.1em]">
                {it.tag}
              </span>
            </li>
          ))}
  ...
  return (
    <section>
      ...
      <p className="text-[12px] leading-[1.5] text-[var(--color-ink-muted)] mb-4 max-w-[68ch]">
        This transaction was matched to a counterparty across the shielded fog.
        The amounts correlate within ZIP-317 fee tolerance — strong evidence
        ...
        {links.map((l, i) => (
          <li
            ...
            style={{
              borderLeftWidth: 2,
              borderLeftColor:
                l.confidence === "HIGH" ? "var(--color-critical)"
                  : l.confidence === "MEDIUM" ? "var(--color-high)"
                    : "var(--color-medium)",
            }}
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-3 items-center">
              <div className="space-y-1">
                ...
                <div className="mono text-[11px] text-[var(--color-deposit)] break-all">
                  {l.senderAddress ?? "—"}
                </div>
                <div className="mono text-[10px] text-[var(--color-ink-dim)]">
                  {shortHex(l.shieldingTxid, 8, 6)}
                </div>
              </div>
              ...
              <div className="space-y-1 lg:text-right">
                ...
                <div className="mono text-[11px] text-[var(--color-withdrawal)] break-all">
                  {l.recipientAddress ?? "—"}
                </div>
                <div className="mono text-[10px] text-[var(--color-ink-dim)]">
                  {shortHex(l.unshieldingTxid, 8, 6)}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mono text-[11px]">
              <span className="display text-[18px] text-ink">
                {zatToZec(l.amountZat)}
                <span className="text-[var(--color-ink-dim)] text-[10px] ml-1">ZEC</span>
              </span>
              <span className="text-[var(--color-ink-muted)]">
                time delta {fmtMs(l.timeDeltaMs)}
              </span>
            </div>
          </li>
        ))}
