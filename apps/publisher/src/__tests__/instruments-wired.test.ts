...
import { fixtureInputs, ZAT_PER_ZEC } from "./harness.js";
...
describe("A1 - the analysis panels are non-null on a published snapshot", () => {
  ...
  it("A1 the panels carry the REAL estimators' arithmetic, not a builder default", () => {
    ...
    // source. One crossing in the fixture window, so `maxWallets` is 1 by Sigma
    // counts, and `denominationRuns` rides beside it as a shape observation.
    ...
    expect(hist.maxWallets).toBe(1);
  });

  it("A1 THE PRODUCTION PATH PUBLISHES TWO OF THE FOUR, and the two absences are the INPUT layer rather than the package move", () => {
    // `readSnapshotInputs` supplies these three shapes on every real tip. They
    // are copied here rather than imported so this assertion fails when that
    // module changes, which is exactly when someone needs to re-read it.
    const productionShaped = fixtureInputs(HEIGHT, {
      orchardSeries: [],
      drainBaseline: null,
      ironwoodSpends: null,
      ironwoodWindow: null,
    });
    const snapshot = buildSnapshot(productionShaped, REAL_INSTRUMENTS);
  ...
  it("A1 NOTED: the real orchardDrain REFUSES a baseline with no series, where the stand-in returned a literal", () => {
    // Not a defect today and it is a trap tomorrow, so it is pinned by name.
    // `buildDrain` calls the estimator whenever a baseline is present, without
    // checking the estimator's own precondition. In production `drainBaseline` is
    // always null, so this cannot fire. The moment a handoff adds the block-time
    // migration and starts supplying a baseline, a tip whose series has no sample
    // at or below the tip height takes the publisher's `build` callback down.
    // The stand-in instruments hid this for a whole handoff because they ignored
    // the series argument entirely.
    const baselineWithoutSeries = fixtureInputs(HEIGHT, { orchardSeries: [] });
    expect(baselineWithoutSeries.drainBaseline).not.toBeNull();
    expect(() => buildSnapshot(baselineWithoutSeries, REAL_INSTRUMENTS)).toThrow(/no sample at or below/);

    // The same inputs with a series do not throw, so the throw is about the
    // series and not about the fixture generally.
    expect(() => buildSnapshot(fullyFedInputs(), REAL_INSTRUMENTS)).not.toThrow();
  });
});
...
describe("A6 - NO_INSTRUMENTS is no longer what the composition root ships", () => {
  ...
  it("A6 the entry point passes REAL_INSTRUMENTS - read from the source, not from a re-export", async () => {
    ...
    expect(source).toMatch(/buildSnapshot\(inputs,\s*REAL_INSTRUMENTS\)/);
    expect(source).not.toMatch(/buildSnapshot\(inputs,\s*NO_INSTRUMENTS\)/);
  });
});
